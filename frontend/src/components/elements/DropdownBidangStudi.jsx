  ...
  return (
    <div ref={mapelRef} className="relative w-full">
      <input
        ...
        value={
          !selectedMapel
            ? t('forms.dataLessons.placeholders.selectSubject')
      ...
      {isOpen && (
        <div className="absolute mt-1  w-full bg-white border z-50 border-gray-400 rounded shadow">
          <div className="sticky top-0    text-xs hover:bg-gray-200 cursor-pointer">
            <input
              type="search"
              placeholder={t('forms.dataLessons.placeholders.searchSubject')}
              value={search}
          ...
          <ul className="max-h-28 overflow-y-auto">
            {dataSearch.length === 0 && (
              ...
              >
                <p>{t('forms.dataLessons.messages.noSubjectsFound')}</p>
              </li>
            )}
            {dataMapel &&
              dataSearch.map((mp) => (
